...
#[\AllowDynamicProperties]
 class ADODB_mongo extends ADOConnection {
	...
	public function fetchAllData($collection,$filter=[],$options =[]){
		$cursor = $this->_db->$collection->find($filter,$options);  //SELECT * FROM $collection WHERE ^^^
		if ($cursor != null) {
	    $return = array();
	    $return = $cursor->toArray();
	 ...
	 *  @return array of arrays
	 ***/
	public function Aggregate($collection,$filter=[],$options,$includematch = true){ 
		if($includematch){
			$data[]=['$match'=>$filter];
		}
	 ...
	 public function toMongoDate ($date=null){
		...
		public function fromMongoDate ($date,$format =null){ 
			if( $date instanceof \MongoDB\BSON\UTCDateTime){
				...
				$date= 	new MongoDB\BSON\UTCDateTime($date['$date']);
			}else{
				$date= 	new MongoDB\BSON\UTCDateTime($date);
			}
			// $date= $date['$date'] ? :	new MongoDB\BSON\UTCDateTime($date);
			$date= $date->toDateTime();
				if ($format){
				$date= $date->format($format);  
				}else{
...
//		return "from_unixtime(unix_timestamp($date)+$fraction)";
	...
	*/
	...
	public function gridfsUpload($collectname,$file,$options =[],$quality=60){
	    try{
	    ...
	    if(is_uploaded_file($file['tmp_name']) && $file['error'] == 0 && !is_array($file['name']) ){
	     ...
	     //   if(in_array($_type_,$ext)){
			...
			//	$bucket = $this->_db->$collectname->selectGridFSBucket();
				...
				$ext = array('image/pjpeg','image/jpeg','image/jpg','image/png','image/x-png','image/gif');
				if(in_array($_type_,$ext)){
					$contents = $this->compressImage($_tmp_name_,$quality);
				}else{
