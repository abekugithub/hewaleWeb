...
class MenuClass extends engineClass
{
	...
	/*
	    * This function loads the Open virtual hospital default menu for the side Dashboard
		*/
	private function getVirtualHospitalViewDashVH()
	{
		$mainmenu = $this->getVirtualHospitalViewVH();
		$stmt = $this->sql->Execute($this->sql->Prepare("SELECT * FROM hmsb_menusubgroupdetail WHERE MENUDET_MENUGPCODE = '012' AND MENUDET_STATUS = '1' AND MENUDET_DASHBOARD = '1' "));
		print $this->sql->ErrorMsg();
		if ($stmt) {
			while ($obj = $stmt->FetchNextObject()) {
				$sidesmenu[] = array($obj->MENUDET_ID, $obj->MENUDET_NAME, $obj->MENUDET_IMAGEUNINAME, $obj->MENUDET_NOTIFICATION, $obj->MENUDET_CODE);
	...
	/*
	   * This function loads the Open Virtual hospital default menu
		 */
	private function getVirtualHospitalViewVH()
	{
		$stmt = $this->sql->Execute($this->sql->Prepare("SELECT * FROM hmsb_menusubgroupdetail WHERE MENUDET_MENUGPCODE = '012' AND MENUDET_STATUS = '1'  "));
		print $this->sql->ErrorMsg();
		if ($stmt) {
			while ($obj = $stmt->FetchNextObject()) {
				$thismenu[] = array($obj->MENUDET_ID, $obj->MENUDET_NAME, $obj->MENUDET_IMAGEUNINAME, $obj->MENUDET_NOTIFICATION, $obj->MENUDET_CODE);
	...
	public function getMenuViewAccessRightDash()
	{
		...
		switch ($this->institutiontype) {
			...
			case "CC":
				...
				break;
			case "VH":
				$mainmenu = $this->getVirtualHospitalViewDashVH();
				break;
		}
