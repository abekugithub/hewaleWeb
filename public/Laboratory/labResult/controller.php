...
if(!empty($fdsearch)){
	...
	$query = "SELECT LTM_VISITCODE,LTM_DATE,LTM_PATIENTNUM,LTM_PATIENTCODE,LTM_PATIENTNAME,LTM_RMK,LTM_ACTORCODE,LTM_ACTORNAME,LTM_INSTCODE,LTM_TOTAL_AMOUNT,LTM_PACKAGECODE FROM hms_patient_labtest_main WHERE  LTM_LABCODE = ".$sql->Param('a')." AND LTM_STATUS IN ('4','5') AND LTM_PAYMENTSTATUS = ".$sql->Param('a')." AND (LTM_PATIENTNAME LIKE ".$sql->Param('c')." OR LTM_PATIENTNUM LIKE ".$sql->Param('d')." OR LTM_PACKAGECODE LIKE ".$sql->Param('e')." ) ";
   $input = array($faccode,'2','%'.$fdsearch.'%','%'.$fdsearch.'%','%'.$fdsearch.'%');

}else {

   //echo $faccode;
   $query = "SELECT LTM_VISITCODE,LTM_DATE,LTM_PATIENTNUM,LTM_PATIENTCODE,LTM_PATIENTNAME,LTM_RMK,LTM_ACTORCODE,LTM_ACTORNAME,LTM_INSTCODE,LTM_TOTAL_AMOUNT,LTM_PACKAGECODE FROM hms_patient_labtest_main WHERE LTM_STATUS IN ('4','5') AND LTM_LABCODE = ".$sql->Param('b')." AND LTM_PAYMENTSTATUS = ".$sql->Param('c');
