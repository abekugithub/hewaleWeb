                ...
                <table class="table table-hover">
                 ...
                 $num = 1;
                  if($paging->total_rows > 0 ){
                   ...
                   echo '<tr >
				   <div align="center">
						<td> <input type="checkbox" value="'.$obj->PRESC_CODE.'" name="syscheckbox['.$obj->PRESC_CODE.']" id="selcheck" > </td>
							</div>
                        <td>'.$num++.'</td>
                        <td>'.date("d/m/Y",strtotime($obj->PRESC_DATE)).'</td>
                        <td>'.$obj->PRESC_PATIENT.'</td>
                        <td>'.$obj->PRESC_PATIENTNUM.'</td>
						<td>'.$obj->PRESC_DRUG.'</td>
                        <td>'.$obj->PRESC_QUANTITY.'</td>
						<td>'.(($obj->PRESC_STATUS == 2)?'Accepted':(($obj->PRESC_STATUS == 3)?'Processed':'')).'</td>
						<td><div class="btn-group">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    <li><button type="submit" onclick="document.getElementById(\'keys\').value=\''.$obj->PRESC_CODE.'\';document.getElementById(\'viewpage\').value=\'\';document.getElementById(\'view\').value=\'process\';document.myform.submit;">Process</button></li>
									
                                </ul>
                            </div>
                        </td>
						<td><button type="submit" class="btn btn-info btn-square" onclick="document.getElementById(\'view\').value=\'add\';document.getElementById(\'keys\').value=\''.$obj->PRESC_CODE.'\';document.getElementById(\'viewpage\').value=\'process\';document.myform.submit;">Process</button></td>					 
