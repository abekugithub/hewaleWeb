...
if (!empty($keys)){
	...
	$stmt = $sql->Execute($sql->Prepare("UPDATE hms_patient_prescription SET PRESC_STATUS = '0' WHERE PRESC_CODE=".$sql->Param('d')),array($keys));
print $sql->ErrorMsg();

    if ($stmt){
        $fetchstmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_patient_prescription WHERE PRESC_STATUS='1' AND PRESC_VISITCODE=".$sql->Param('a')." AND PRESC_PATIENTCODE=".$sql->Param('b')." ORDER BY PRESC_ID DESC"),array($visitcode,$patientcode));
        print $sql->ErrorMsg();
        $result = array();
        $num = 1;
        if ($fetchstmt->RecordCount()>0){
            while ($obj = $fetchstmt->FetchNextObject()){
                ...
                $result[]=array("<tr><td>".$num++."</td><td>".$drugname."</td><td>".$obj->PRESC_DAYS."</td><td>".$obj->PRESC_FREQ."</td><td>".$obj->PRESC_TIMES."</td><td><button type='button' id='deleteprescription' onclick='deleteprescription(\"".$obj->PRESC_CODE."\",\"Prescription\");' class=\"btn-danger removecomplain\">&times;</button></td></tr>");
            }
        }
    }/*else{
        ...
        if ($stmtpresc){
            ...
            if ($fetchstmt->RecordCount()>0){
				...
				$encaes = new encAESClass($saltencrypt,'CBC',$pepperdecrypt);
					...
					$drugname = $encaes->decrypt($obj->PRESC_DRUG);
                    $result[]=array("<tr><td>".$num++."</td><td>".$drugname."</td><td>".$obj->PRESC_DAYS."</td><td>".$obj->PRESC_FREQ."</td><td>".$obj->PRESC_TIMES."</td><td><button type='button' id='deletecomplain' onclick='deleteComplains(\"".$obj->PRESC_CODE."\",\"Prescription\");' class=\"btn-danger removecomplain\">&times;</button></td></tr>");
                }
            }
        }
    }*/
