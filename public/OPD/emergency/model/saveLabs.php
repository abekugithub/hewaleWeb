...
if (!empty($testcode)&&!empty($testname)){
	$testname = $encaes->encrypt($testname);
	$testcode = $encaes->encrypt($testcode);
    ...
    if ($labscheck->RecordCount()>0){
        ...
        if (isset($activeinstitution) && !empty($activeinstitution)){
            $stmtlab = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_labtest (LT_CODE,LT_VISITCODE,LT_DATE,LT_PATIENTNUM,LT_PATIENTNAME,LT_TEST,LT_TESTNAME,LT_DISCIPLINE,LT_DISCPLINENAME,LT_RMK,LT_ACTORCODE,LT_ACTORNAME,LT_INSTCODE,LT_PATIENTCODE,LT_ENCRYPKEY,LT_LABCODE,LT_LABNAME,LT_STATUS) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').",".$sql->Param('13').",".$sql->Param('13').",".$sql->Param('14').",".$sql->Param('15').",".$sql->Param('16').",".$sql->Param('17').")"),
                array($lqcode,$visitcode,$day,$patientnum,$patientname,$testcode,$testname,$dicscode,$dicsname,$remark,$usrcode,$usrname,$activeinstitution,$patientcode,$encryptkey,$activeinstitution,$activeinstitution,'6'));
            ...
            $stmtlab = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_labtest (LT_CODE,LT_VISITCODE,LT_DATE,LT_PATIENTNUM,LT_PATIENTNAME,LT_TEST,LT_TESTNAME,LT_DISCIPLINE,LT_DISCPLINENAME,LT_RMK,LT_ACTORCODE,LT_ACTORNAME,LT_INSTCODE,LT_PATIENTCODE,LT_ENCRYPKEY) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').",".$sql->Param('13').",".$sql->Param('13').",".$sql->Param('14').")"),
                array($lqcode,$visitcode,$day,$patientnum,$patientname,$testcode,$testname,$dicscode,$dicsname,$remark,$usrcode,$usrname,$activeinstitution,$patientcode,$encryptkey));
        }
        print $sql->ErrorMsg();
        if ($stmtlab){
            ...
            $fetchstmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_patient_labtest WHERE LT_STATUS IN ('1','6') AND LT_VISITCODE=".$sql->Param('a')." AND (LT_PATIENTNUM=".$sql->Param('b')." OR LT_PATIENTCODE=".$sql->Param('b').") ORDER BY LT_ID DESC"),array($visitcode,$patientnum,$patientcode));
            print $sql->ErrorMsg();
            $result = array();
            $num = 1;
            if ($fetchstmt->RecordCount()>0){
