...
if (!empty($reportarea)){
	...
	$stmtr = $sql->Execute($sql->Prepare("INSERT INTO hms_emergency_reports (REP_EMERCODE,REP_VISITCODE,REP_DETAILS,REP_PATIENTNUM,REP_PATIENTCODE,REP_ACTOR,REP_ACTORID,REP_FACICODE,REP_DATE) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').")"),array($emergencycode,$visitcode,$reportarea,$patientnum,$patientcode,$usrname,$usrcode,$faccode,$date));
    print $sql->ErrorMsg();
    if ($stmtr){
