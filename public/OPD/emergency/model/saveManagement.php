...
if (!empty($txtarea)){
	...
	$stmtman = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_management (PM_MANAGEMENT,PM_CODE,PM_DATE,PM_PATIENTNUM,PM_VISITCODE,PM_INSTCODE,PM_ACTORNAME,PM_ACTORCODE,PM_ENCRYPKEY) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').")"),array($txtarea,$managementcode,$day,$patientnum,$visitcode,$activeinstitution,$usrname,$usrcode,$encryptkey));
    print $sql->ErrorMsg();
    if ($stmtman){
