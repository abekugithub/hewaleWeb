...
switch($viewpage){
	...
	//echo $userCourier;exit;
		$stmt = $sql->Execute($sql->Prepare("SELECT COU_CODE,COU_FNAME, COU_SURNAME,COU_CONTACT,COU_EMAIL,COU_ADDRESS,COU_DOB, COU_UNAME FROM hms_courieragents  WHERE COU_CODE  = ".$sql->Param('a')." "),array($keys)); 
		print $sql->ErrorMsg();
		if($stmt->RecordCount() == 1){
			$obj = $stmt->FetchNextObject();
			$fname = $obj->COU_FNAME;
			$othername = $obj->COU_SURNAME;
			$phonenumber = $obj->COU_CONTACT;
			$address = $obj->COU_ADDRESS;
			$email = $obj->COU_EMAIL;
			$startdate = $obj->COU_DOB;
...
if($postkey != $microtime){
	...
	if( empty($fname) || empty($othername) || empty($email) || empty($phonenumber) || empty($address) || empty($startdate)){
		...
		$stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_courieragents WHERE COU_FNAME=".$sql->Param('a')." AND COU_SURNAME = ".$sql->Param('b')."  "),array($fname, $othername));
			print $sql->ErrorMsg();
		...
		if($stmt->RecordCount()>0){
			...
			$agentdob = date("Y-m-d",strtotime($startdate));
			$newfirstname = strtoupper($fname);
			$newsurname = strtoupper($othername);
			$agentcode = uniqid();
			
			$sql->Execute($sql->Prepare("INSERT INTO hms_courieragents (COU_CODE,COU_FNAME, COU_SURNAME,COU_CONTACT,COU_EMAIL,COU_ADDRESS,COU_DOB,COU_INSTCODE, COU_UNAME, COU_PICNAME, COU_PICSIZE) VALUES (".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').", ".$sql->Param('e').", ".$sql->Param('f').", ".$sql->Param('g').", ".$sql->Param('h').", ".$sql->Param('i').", ".$sql->Param('j').", ".$sql->Param('k').")"), array($agentcode,$newfirstname,$newsurname, $phonenumber,$email, $address,$agentdob,$userCourier,$neu_name,$_name_,$_size_));
print $sql->ErrorMsg();
...
array($neu_name,$_name_,$_size_,$agentcode));
	print $sql->ErrorMsg();	
...
if(!empty($fdsearch)){
    ...
    $query = "SELECT COU_CODE,COU_FNAME, COU_SURNAME,COU_CONTACT,COU_EMAIL,COU_ADDRESS,COU_DOB,COU_INSTCODE,COU_STATUS FROM hms_courieragents WHERE COU_STATUS ='1' AND COU_INSTCODE=".$sql->Param('a')." "; $input = array($userCourier);
}
if(!isset($limit)){
