...
switch($viewpage){
	...
	if(empty($price) || empty($accommodation)  || empty($paymeth)){
	...
	print $sql->ErrorMsg();
		...
		$sql->Execute("INSERT INTO hmsb_st_pricing(PS_CODE,PS_CATEGORY,PS_ITEMCODE,PS_ITEMNAME,PS_PRICE,PS_PAYSCHEME,PS_PAYSCHEMENAME,PS_USERCODE,PS_USERFULLNAME,PS_FACICODE) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').",".$sql->Param('e').",".$sql->Param('f').",".$sql->Param('g').",".$sql->Param('h').",".$sql->Param('j').",".$sql->Param('k').")",array($ttcode,'4',$scode,$sname,$price,$pcode,$pname,$actorid,$actorname,$activeinstitution));
		print $sql->ErrorMsg();
...
$stmtaccommodationlov = $sql->Execute($sql->prepare("SELECT * from hms_st_wardbed WHERE BED_STATUS = '0'"));
