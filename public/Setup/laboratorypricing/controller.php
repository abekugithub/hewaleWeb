...
switch($viewpage){
    case "saveprice":
        if(empty($price) || empty($labtest)  || empty($paymeth)){
            ...
            if($stmt->RecordCount() > 0){
                ...
                if (empty($keys)){
                    ...
                    $sql->Execute("INSERT INTO hmsb_st_pricing(PS_CODE,PS_CATEGORY,PS_ITEMCODE,PS_ITEMNAME,PS_PRICE,PS_PAYSCHEME,PS_PAYSCHEMENAME,PS_USERCODE,PS_USERFULLNAME,PS_FACICODE) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').",".$sql->Param('e').",".$sql->Param('f').",".$sql->Param('g').",".$sql->Param('h').",".$sql->Param('j').",".$sql->Param('k').")",array($ttcode,'1',$scode,$sname,$price,$pcode,$pname,$actorid,$actorname,$activeinstitution));
                    print $sql->ErrorMsg();
...
//                $keys = $obj->PS_CODE;
	...
	case "reset":
	   $fdsearch = $action_search = $keys = "";
	break;
}

$stmtpayschemelov = $sql->Execute($sql->prepare("SELECT * from hmsb_st_paymentmethod WHERE PAYM_FACICODE = '$activeinstitution'"));
$stmtservicelov = $sql->Execute($sql->prepare("SELECT * from hmsb_st_labtest WHERE LTT_STATUS = '1'"));
