...
switch($viewpage){
    case "saveprice":
        if(empty($price) || empty($xray)  || empty($paymeth)){
            ...
            if($stmt->RecordCount() > 0){
                ...
                if (empty($keys)){
                    ...
                    $sql->Execute("INSERT INTO hmsb_st_pricing(PS_CODE,PS_CATEGORY,PS_ITEMCODE,PS_ITEMNAME,PS_PRICE,PS_PAYSCHEME,PS_PAYSCHEMENAME,PS_USERCODE,PS_USERFULLNAME,PS_FACICODE) VALUES(".$sql->Param('a').",".$sql->Param('b').",".$sql->Param('c').",".$sql->Param('d').",".$sql->Param('e').",".$sql->Param('f').",".$sql->Param('g').",".$sql->Param('h').",".$sql->Param('j').",".$sql->Param('k').")",array($ttcode,'3',$scode,$sname,$price,$pcode,$pname,$actorid,$actorname,$activeinstitution));
                    print $sql->ErrorMsg();
...
$stmtservicelov = $sql->Execute($sql->prepare("SELECT * from hmsb_st_xray WHERE X_STATUS = '1'"));
