    ...
    <div class="page-wrapper">
        <div class="page form">
            <div class="moduletitle">
                <div class="moduletitleupper">Prescription Details<span class="pull-right">
			...
			$stmt = $sql->Execute($sql->Prepare("SELECT PRESC_DATE FROM hms_patient_prescription WHERE
 PRESC_STATUS = '6' AND PRESC_PATIENTNUM=".$sql->Param('a')." AND PRESC_VISITCODE=".$sql->Param('b')." GROUP BY PRESC_DATE "),array($patientno[0],$keys_code[1]));
			
			if($stmt->RecordCount()>0){
		while($obj=$stmt->FetchNextObject()){
			 ?>
             <div><b>Prescription Date:</b> <?php $createDate = new DateTime($obj->PRESC_DATE);

$mydate = $createDate->format('d-M-Y');
  echo $mydate;  ?></div>
             
             <?php 
			$stmts = $sql->Execute($sql->Prepare("SELECT PRESC_CODE, PRESC_PATIENT,PRESC_PATIENTCODE,PRESC_PATIENTNUM,PRESC_DATE,PRESC_VISITCODE,PRESC_DRUG,PRESC_QUANTITY,PRESC_DOSAGENAME,PRESC_FREQ,PRESC_DAYS,PRESC_TIMES FROM hms_patient_prescription WHERE PRESC_STATUS = ".$sql->Param('a')." AND PRESC_VISITCODE=".$sql->Param('b')." AND PRESC_DATE=".$sql->Param('c')." "),array('6',$keys_code[1],$obj->PRESC_DATE));
	 ...
	 while($objs=$stmts->FetchNextObject()){ 
							 ?>
                                <tr>
                                    <td><?php echo $encaes->decrypt($objs->PRESC_DRUG);?></td>
                                    <td><?php echo $objs->PRESC_DAYS;?></td>
                                     <td><?php echo $objs->PRESC_FREQ;?></td>
                                    <td><?php echo $objs->PRESC_TIMES;?></td>
                                </tr>
                                 <?php  }}?>
                ...
                <div id="message"></div>
                    ...
                    <div class="col-sm-4 required">
                        <label for="othername">Prescription</label>
                    ...
                    <div class="col-sm-4 required">Days
                        <input type="numbers" class="form-control" id="days" name="days" placeholder="Days">
                    </div>
                    <div class="col-sm-4 required">Frequency
                        <input type="numbers" class="form-control" id="frequency" name="frequency" placeholder="Frequency">                    </div>
                        <div class="col-sm-4 required">Times
                        <input type="numbers" class="form-control" id="times" name="times" placeholder="Times">                        </div>
                    <div class="btn-group">
                        <div class="col-sm-4 ">
                            <label for="othername">&nbsp;</label>
                            <button type="button" onclick="saveprescription();" class="btn btn-success"><i class="fa fa-plus-circle"></i> Submit</button>
                        </div>
                    </div>
                    </div>
                </div>
