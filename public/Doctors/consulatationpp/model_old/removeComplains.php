...
if (!empty($comcode)&&!empty($patientnum)&&!empty($patientcode)&&!empty($visitcode)){
    if ($type === 'Complains'){
        ...
        if ($stmt){
            ...
            if ($fetchstmt->RecordCount()>0){
                while ($obj = $fetchstmt->FetchNextObject()){
                    $result[]=array("<tr><td>".$num++."</td><td>".$encaes->decrypt($obj->DIA_DIAGNOSIS)."</td><td>".$encaes->decrypt($obj->DIA_RMK)."</td><td>".(($obj->DIA_TYPE == 1)?'Final':'Provisional')."</td><td><button type='button' id='deletecomplain' onclick='deleteComplains(\"".$obj->DIA_CODE."\",\"Diagnosis\");' class=\"btn-danger removecomplain\">&times;</button></td></tr>");
                }
            }else{
                $result[] = array("");
            }
        ...
        //Get prescription's package code
        $stmtpck = $sql->Execute($sql->Prepare("SELECT PRESC_PACKAGECODE FROM hms_patient_prescription WHERE PRESC_CODE=".$sql->Param('a')." "),array($comcode));
        $objpck = $stmtpck->FetchNextObject();
        $packcode = $objpck->PRESC_PACKAGECODE;

        //Check if the prescription is the only one left
        $stmtrec = $sql->Execute($sql->Prepare("SELECT * FROM hms_patient_prescription WHERE PRESC_PACKAGECODE=".$sql->Param('a')." AND PRESC_CODE !=".$sql->Param('b')." AND PRESC_STATUS !='0' "),array($packcode,$comcode));
        if($stmtrec->RecordCount() == 0){
            //If there is no other pending drug set status in main prescription table to 0
            $sql->Execute($sql->Prepare("UPDATE hms_patient_prescription_main SET PRESCM_STATUS='0' WHERE  PRESCM_PACKAGECODE = ".$sql->Param('a')." AND PRESCM_STATUS IN ('1','2') "),array($packcode));
        print $sql->ErrorMsg();
        }


        $stmt = $sql->Execute($sql->Prepare("UPDATE hms_patient_prescription SET PRESC_STATUS='0' WHERE PRESC_CODE=".$sql->Param('a')." AND (PRESC_PATIENTNUM=".$sql->Param('a')." OR PRESC_PATIENTCODE=".$sql->Param('a').") AND PRESC_VISITCODE=".$sql->Param('a')." AND PRESC_STATUS IN ('1','2') "),array($comcode,$patientnum,$patientcode,$visitcode));
        print $sql->ErrorMsg();
