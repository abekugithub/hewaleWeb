 ...
 */
$presmain = $sql->Execute($sql->Prepare("SELECT PRESCM_ITEMCODE FROM hms_patient_prescription_main WHERE PRESCM_PACKAGECODE = ".$sql->Param('a')." "),array($prespackagecode));

print $sql->ErrorMsg();
if($presmain->RecordCount() == 0 ){
    $presmid = uniqid();
    $stmtpresc = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_prescription_main (PRESCM_ID,PRESCM_PATIENT,PRESCM_PATIENTNUM,PRESCM_DATE,PRESCM_VISITCODE,PRESCM_ACTORNAME,PRESCM_ACTORCODE,PRESCM_INSTCODE,PRESCM_PATIENTCODE,PRESCM_ENCRYPKEY,PRESCM_PACKAGECODE,PRESCM_PATIENTCONTACT,PRESCM_ITEMCODE,PRESCM_COUNTRYCODE) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').",".$sql->Param('13').",".$sql->Param('14').")"),
    array($presmid,$patientname,$patientnum,$day,$visitcode,$usrname,$usrcode,$activeinstitution,$patientcode,$encryptkey,$prespackagecode,$phonenum,$itemcode,$countrycode));
print $sql->ErrorMsg();
...
$stmtb=$sql->Execute($sql->Prepare("INSERT INTO hms_broadcast_prescription (BRO_CODE, BRO_PRESCCODE, BRO_VISITCODE, BRO_PATIENTNAME, BRO_PATIENTCODE, BRO_PATIENTNUM, BRO_PATIENTCONTACT, BRO_PHARMACYNAME, BRO_PHARMACYCODE, BRO_DATE, BRO_LONGITUDE, BRO_LATITUDE,BRO_STATE,BRO_COUNTRYCODE) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').",".$sql->Param('13').",".$sql->Param('14').")"),array($Code, $prespackagecode, $visitcode, $patientname, $patientcode, $patientnum, $phonenum, $pharmacy_name, $facicode, $day,$longitude,$latitude,$pressource,$countrycode));
print $sql->ErrorMsg();
...
print $sql->ErrorMsg();
    ...
    if ($prescriptioncheck->RecordCount()>0){
        ...
        $stmtpresc = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_prescription (PRESC_ID,PRESC_CODE,PRESC_PATIENT,PRESC_PATIENTNUM,PRESC_DATE,PRESC_VISITCODE,PRESC_DRUGID,PRESC_DRUG,PRESC_QUANTITY,PRESC_DOSAGENAME,PRESC_DOSAGECODE,PRESC_FREQ,PRESC_DAYS,PRESC_TIMES,PRESC_ACTORNAME,PRESC_ACTORCODE,PRESC_INSTCODE,PRESC_PATIENTCODE,PRESC_ENCRYPKEY,PRESC_ROUTECODE,PRESC_ROUTENAME,PRESC_PACKAGECODE,PRESC_PATIENTCONTACT,PRESC_ITEMCODE,PRESC_COUNTRYCODE) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').",".$sql->Param('13').",".$sql->Param('14').",".$sql->Param('15').",".$sql->Param('16').",".$sql->Param('16').",".$sql->Param('17').",".$sql->Param('18').",".$sql->Param('19').",".$sql->Param('20').",".$sql->Param('21').",".$sql->Param('22').",".$sql->Param('23').",".$sql->Param('24').")"),
            array($presid,$precode,$patientname,$patientnum,$day,$visitcode,$drugcode,$drugname,$qty,$drugdosename,$drugdose,$frequency,$days,$times,$usrname,$usrcode,$activeinstitution,$patientcode,$encryptkey,$routecode,$routename,$prespackagecode,$phonenum,$icode,$countrycode));
        print $sql->ErrorMsg();
