...
$stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_consultation WHERE CONS_FACICODE = ".$sql->Param('a')."  AND  CONS_DOCTORCODE = ".$sql->Param('b')." AND CONS_STATUS IN ('1','2') ORDER BY CONS_LASTMODIFIED DESC"),array($activeinstitution,$usrcode));
//print $sql->ErrorMsg();
...
$medium = (($requestdetails->REQU_CONSULTTYPE == '1')?'Text <i class="fa fa-comments-o" style="float:right;font-size:20px;color:red"></i>':(($requestdetails->REQU_CONSULTTYPE == '2')?'Audio <i class="fa fa-phone" style="float:right;font-size:20px;color:green"></i>':(($requestdetails->REQU_CONSULTTYPE == '3')?'Video <i class="fa fa-file-video-o" style="float:right;font-size:20px;color:orange"></i>':'Text <i class="fa fa-comments-o" style="float:right;font-size:20px;color:red"></i>')));
