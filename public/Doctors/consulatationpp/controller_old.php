...
$stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_consultation WHERE CONS_CODE = ".$sql->Param('a')." "),array($key[0]));
	...
	case "saveconsultation":
		   ...
		   //End update for active consultation set
                ...
                $stmtqueue = $sql->Execute($sql->Prepare("SELECT * FROM hms_consultation_queue WHERE CONS_DOCTORCODE IS NULL AND CONS_COUNTRYCODE = ".$sql->Param('a')." ORDER BY CONS_INPUTDATE ASC LIMIT 1"),array($countrycode));
                print $sql->ErrorMsg();
                ...
                $sql->Execute($sql->Prepare("INSERT INTO hms_consultation(CONS_DATE,CONS_PATIENTNUM,CONS_PATIENTCODE,CONS_PATIENTNAME,CONS_VISITCODE,CONS_SERVICECODE,CONS_SERVICENAME,CONS_DOCTORNAME,CONS_DOCTORCODE,CONS_STATUS,CONS_COST,CONS_FACICODE,CONS_SERVCODE,CONS_PAYMETHCODE,CONS_PAYMETH,CONS_PAYSTATE)
  SELECT CONS_DATE,CONS_PATIENTNUM,CONS_PATIENTCODE,CONS_PATIENTNAME,CONS_VISITCODE,CONS_SERVICECODE,CONS_SERVICENAME,CONS_DOCTORNAME,CONS_DOCTORCODE,CONS_STATUS,CONS_COST,CONS_FACICODE,CONS_SERVCODE,CONS_PAYMETHCODE,CONS_PAYMETH,CONS_PAYSTATE FROM hms_consultation_queue WHERE CONS_VISITCODE = ".$sql->Param('a')." "),array($objqueue->CONS_VISITCODE));
...
// userlog event
    ...
    case "loadqueuedpatient":
                    ...
                    $stmtqueue = $sql->Execute($sql->Prepare("SELECT * FROM hms_consultation_queue WHERE CONS_DOCTORCODE IS NULL ORDER BY CONS_INPUTDATE ASC LIMIT 1"));
                    print $sql->ErrorMsg();
    ...
    $sql->Execute("UPDATE hms_consultation SET CONS_STATUS = '1',CONS_CODE = ".$sql->Param('a').",CONS_REQUCODE = ".$sql->Param('b').",CONS_FACICODE = ".$sql->Param('c')." WHERE CONS_VISITCODE = ".$sql->Param('d')." ",array($conscode,$requcode,$activeinstitution,$objqueue->CONS_VISITCODE));
      print $sql->ErrorMsg();
...
case "loadqueuedpatientappt":
    ...
    if($stmtqueue->RecordCount() > 0){
        ...
        $sql->Execute($sql->Prepare("INSERT INTO hms_consultation(CONS_DATE,CONS_PATIENTNUM,CONS_PATIENTCODE,CONS_PATIENTNAME,CONS_VISITCODE,CONS_SERVICECODE,CONS_SERVICENAME,CONS_DOCTORNAME,CONS_DOCTORCODE,CONS_FACICODE,CONS_SERVCODE,CONS_PAYMETHCODE,CONS_PAYMETH,CONS_PAYSTATE) SELECT REQU_DATE,REQU_PATIENTNUM,REQU_PATIENTCODE,REQU_PATIENT_FULLNAME,REQU_VISITCODE,REQU_SERVICECODE,REQU_SERVICENAME,REQU_DOCTORNAME,REQU_DOCTORCODE,REQU_FACI_CODE,REQU_SERVICECODE,REQU_PAYMETHCODE,REQU_PAYMETH,REQU_PAYSTATE FROM hms_service_request WHERE REQU_VISITCODE = ".$sql->Param('a')." "),array($objqueue->REQU_VISITCODE));

        print $sql->ErrorMsg();

        $sql->Execute("UPDATE hms_consultation SET CONS_STATUS = '1',CONS_CODE = ".$sql->Param('a').",CONS_REQUCODE = ".$sql->Param('b').",CONS_FACICODE = ".$sql->Param('c')." WHERE CONS_VISITCODE = ".$sql->Param('d')." ",array($conscode,$requcode,$activeinstitution,$objqueue->REQU_VISITCODE));
        print $sql->ErrorMsg();
...
$referralHospitals = $sql->Execute($sql->Prepare("SELECT * FROM `socialhealthdb`.`hmsc_refer_hospitals` WHERE `REF_HOSP_DIRECTORATE` = ? OR `REF_HOSP_SUB_DIRECTORATE` = ? OR `REF_HOSP_VCENTER` = ? OR `REF_HOSP_VUNIT` = ? "),[$actorvcode,$actorvcode,$actorvcode,$actorvcode]) ;
