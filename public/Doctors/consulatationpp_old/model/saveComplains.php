...
if (!empty($complaincode)){ // &&!empty($complain)
    ...
    if ($complaincheck->RecordCount() > 0){
        ...
        $stmt = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_complains (PC_CODE,PC_PATIENTNUM,PC_VISITCODE,PC_DATE,PC_COMPLAINCODE,PC_COMPLAIN,PC_INSTCODE,PC_ACTORCODE,PC_ACTORNAME,PC_PATIENTCODE,PC_STORYLINE,PC_ENCRYPKEY) VALUES(".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').",".$sql->Param('12').")"),
            array($comcode,$patientnum,$visitcode,$day,$complaincode,$complain,$activeinstitution,$usrcode,$usrname,$patientcode,$storyline,$encryptkey));
        print $sql->ErrorMsg();
        if ($stmt){
...
//    $complaincode = $engine->generateCode_bk('hmsb_st_symtoms','SY','SY_CODE');
    ...
    $stmt = $sql->Execute($sql->Prepare("INSERT INTO hmsb_st_symtoms (SY_CODE, SY_DESC, SY_NAME, SY_ACTOR, SY_DATE_CREATED, SY_STATUS) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('5').")"),array($complaincode,'',$complain,$actorcode,$day,'2'));
    print $sql->ErrorMsg();
    if ($stmt){
        $complain = $encaes->encrypt(trim($complain));
        $complaincode = $encaes->encrypt(trim($complaincode));
