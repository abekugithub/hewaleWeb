...
if (!empty($diagnosiscode)&&!empty($diagnosisname)){
	$diagnosisname = $encaes->encrypt(trim($diagnosisname));
	$diagnosiscode = $encaes->encrypt(trim($diagnosiscode));
    ...
    if ($diagnosischeck->RecordCount()>0){
        ...
        if ($fetchstmt->RecordCount()>0){
	        ...
	        $diagremark = $encaes->decrypt($obj->DIA_RMK);
                $result[]=array("<tr><td>".$num++."</td><td>".$diagnosisname."</td><td>".$diagremark."</td><td>".(($obj->DIA_TYPE == 1)?'Final':'Provisional')."</td><td><button type='button' id='deletecomplain' onclick='deleteComplains(\"".$obj->DIA_CODE."\",\"Diagnosis\");' class=\"btn-danger removecomplain\">&times;</button></td></tr>");
            }
        }
    }else{
        $stmtdiag = $sql->Execute($sql->Prepare("INSERT INTO hms_patient_diagnosis (DIA_CODE,DIA_VISITCODE,DIA_DATE,DIA_PATIENTNUM,DIA_DIA,DIA_DIAGNOSIS,DIA_RMK,DIA_ACTORNAME,DIA_ACTORCODE,DIA_INSTCODE,DIA_PATIENTCODE,DIA_ENCRYPKEY,DIA_TYPE) VALUES (".$sql->Param('1').",".$sql->Param('2').",".$sql->Param('3').",".$sql->Param('4').",".$sql->Param('5').",".$sql->Param('6').",".$sql->Param('7').",".$sql->Param('8').",".$sql->Param('9').",".$sql->Param('9').",".$sql->Param('9').",".$sql->Param('10').",".$sql->Param('11').")"),
            array($diacode,$visitcode,$day,$patientnum,$diagnosiscode,$diagnosisname,$remark,$usrname,$usrcode,$activeinstitution,$patientcode,$encryptkey,$diatype));
        print $sql->ErrorMsg();
        if ($stmtdiag){
