...
$actor = $engine->getActorDetails();
    ...
    $stmtpre = $sql->Execute($sql->Prepare("SELECT PATIENT_CHRONIC_CONDITION FROM hms_patient WHERE PATIENT_PATIENTNUM = ".$sql->Param('a')." "),array($patientnum));
    $objpre = $stmtpre->FetchNextObject();

    $stmtpos = $sql->Execute("UPDATE hms_patient SET PATIENT_CHRONIC_CONDITION = '' WHERE PATIENT_PATIENTNUM = ".$sql->Param('a')." ",array($patientnum) );

    //Save event log
    $activity = "Chroniques conditions deleted for patient with Hewale Number: ".$patientnum." by doctor ".$usrname." using code ".$usrcode." The details of the chroniques conditions were: ".$objpre->PATIENT_CHRONIC_CONDITION;
    $engine->setEventLog("117",$activity);
