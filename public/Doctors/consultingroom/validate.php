...
/********************************************
 * ******************************************
 * **This php script is intentionally kept***
 * **in the validate for fetching and *******
 * **inserting all data process in the ****** 
 * **iframe**********************************
 * ******************************************
 * ******************************************
 */
...
$objfacdetails = $engine->getUserFacility($usrcode);
$facicode = $objfacdetails->FACI_CODE;
...
$facility_code =  $engine->getActorDetails()->USR_FACICODE;
...
$activeinstitution = $engine->getActorDetails()->USR_FACICODE;
...
$stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_consultation WHERE CONS_CODE = ".$sql->Param('a')." "),array($key[0]));
	...
	$msg = "Patient Consultation request has been cancelled Successfully.";
	    $status = "success";
		$engine->ClearNotification('0196',$cons->CONS_ID);
	...
	case "saveconsultation":
            ...
            if($consstatus[0] == 'SER0021'){ 
            	...
            	$stmt =$sql->Execute($sql->Prepare("SELECT REF_HOSP_TELEPHONE from hmsc_refer_hospitals WHERE REF_HOSP_CODE=".$sql->Param('a').""),array($refhosdetails[0]));
                if ($stmt->RecordCount()>0){
                	$obj=$stmt->FetchNextObject();
                	$number=$obj->REF_HOSP_TELEPHONE;
                }else{
                	$number='';
                }
                $pmessage="Dr $usrname has referred a patient to your facility";
            	 $number = $sms->validateNumber($number,'+233');
                 $sms->sendSms($number,$pmessage);
                //var_dump($refhosdetails[1]);die;
               // $sql->debug=true;
                $sql->Execute("UPDATE hms_consultation SET CONS_REFERD_HOSP_CODE= ?,CONS_REFERD_HOSP_NAME= ?, CONS_STATUS = ?, CONS_SERVCODE = ?, CONS_SERVICECODE = ? WHERE CONS_VISITCODE = ? ",[$refhosdetails[0],$refhosdetails[1],'12','SER0021','SER0021',$visitcode]);
                 //   exit;
            ...
            $sql->Execute("UPDATE hms_consultation SET CONS_SERVCODE = ".$sql->Param('a').", CONS_STATUS = ".$sql->Param('b').",CONS_DOCTORNAME = ".$sql->Param('c').",CONS_DOCTORCODE = ".$sql->Param('d')." WHERE CONS_VISITCODE = ".$sql->Param('e')." ",array($consstatus[0],$consstatus[1],$usrname,$usrcode,$visitcode));
            }else{
            ...
            //$objserv = $patientCls->getServRequestDetail($consstatus[0]);
		    $menudetailscode = '0196';
		    ...
		    if (!empty($consstatus)){
                ...
                if($consstatus[0] == 'SER0004'){    //Vitals
					...
					$menudetailscode = '0000'; //No notification to other module
                }elseif($consstatus[0] == 'SER0021'){   //Hospital Referal 
                    $code = '052';
                    $desc = "Patient has been referred to another hospital.";
					$menudetailscode = '0220'; //This is the referral patient list module in the referred hospital
                }else{
                    $code = '016' ;
                    $objprsc = $patientCls->getPrescriptionInfo($visitcode);
                    if(!empty($objprsc->PRESC_PATIENTNUM)){
                ...
                $tablerowid = (($code == '052')?$tablerowid:$smtrequstdetails->REQU_ID);
                $sentto = $patientcode;

                //This code is triggered if the service requested is a referral to another facility.
                if($consstatus[0] == 'SER0021'){ $sentto = $facilityin = $refhosdetails[0] ;}
                
                $engine->setNotification($code,$desc,$menudetailscode,$tablerowid,$sentto,$facilityin);
                
                $consmenudetailscode = '0196';
                $engine->ClearNotification($consmenudetailscode,$tablerowid);
...
// userlog event
	...
	if(isset($keys) && !empty($keys)){
        ...
        $patientbloodgrp = $patientCls->getPatientDetails($patientnum)->PATIENT_BLOODGROUP;
        $patientweight = $patientCls->getPatientDetails($patientnum)->PATIENT_WEIGHT;
 ...
 //$sql->debug = true;
    $stmthl = $sql->Execute($sql->Prepare( "SELECT * FROM hms_patient_labtest_main WHERE LTM_ACTORCODE = ".$sql->Param('a')." AND LTM_INSTCODE = ".$sql->Param('b')." AND LTM_STATUS != ".$sql->Param('c')." AND LTM_PATIENTNUM =  ".$sql->Param('c')." "),[$actor_id,$facility_code,'0',$keys]) ;
...
if ($stmtlisttestdetails->RecordCount()>0){
	...
	$stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_patient WHERE PATIENT_PATIENTCODE=".$sql->Param('a')." "),array($patientcode));
		...
		$stmtv = $sql->Execute($sql->Prepare("SELECT * FROM hms_vitals_details WHERE VITDET_VISITCODE=".$sql->Param('a')." "),array($keys));
...
//echo $usrcode;
if(!empty($fdsearch)){
 $query = "SELECT * FROM hms_consultation WHERE CONS_FACICODE = ".$sql->Param('a')." AND CONS_STATUS = '1' AND (CONS_PATIENTNUM = ".$sql->Param('c')." OR CONS_PATIENTNAME LIKE ".$sql->Param('d').") ORDER BY CONS_INPUTDATE DESC";
    $input = array($activeinstitution,$fdsearch,'%'.$fdsearch.'%');
}else {
  $query = "SELECT * FROM hms_consultation WHERE CONS_FACICODE = ".$sql->Param('a')." AND CONS_STATUS = '1' ORDER BY CONS_INPUTDATE DESC";
...
$referralHospitals = $sql->Execute($sql->Prepare("SELECT * FROM hmsb_chps_referalhosp_assignments WHERE CHA_HOSPITAL_FACCODE = ? "),[$activeinstitution]) ;

//get pending patient in queue
$stmtqueue = $sql->Execute($sql->Prepare("SELECT COUNT(*) AS TOTALPATIENT FROM hms_consultation_queue WHERE CONS_DOCTORCODE IS NULL"));
if($stmtqueue->RecordCount() > 0){
$objqueue = $stmtqueue->FetchNextObject();
...
//Fetch empty room and update doctor's room when necessary
$doctors->setDocRoom($facicode,$usrcode);

//Get Assigned room to doctor
$stmtroom = $doctors->getDocRoom($facicode,$usrcode);
$objroom = $stmtroom->FetchNextObject();
$roomname = $objroom->CONSROOM_NAME ;
