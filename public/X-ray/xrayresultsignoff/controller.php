...
switch ($viewpage){
	...
	case "reject":
		...
		foreach($_POST["syscheckbox"] as $keysd){
			...
			$stmt = $sql->Execute($sql->Prepare("UPDATE hms_patient_xraytest SET XT_STATUS =".$sql->Param('a')."  WHERE XT_CODE=".$sql->Param('5')." "),array('8',$keysd));
			print $sql->ErrorMsg();
			...
			//	die('tested the things');
				...
				$stmtlabmain = $sql->Execute($sql->Prepare("Update hms_patient_xraytest_main set XTM_STATUS = ".$sql->Param('1')." where XTM_PACKAGECODE = ".$sql->Param('2')." "),array('8',$packagecode));
				print $sql->Errormsg();			
...
if(!empty($fdsearch)){
    ...
    $input = array($faccode,'6','1','%'.$fdsearch.'%','%'.$fdsearch.'%');
