...
switch ($viewpage){
    ...
    case 'resultsss':
	    if (isset($target) && !empty($target)){
	        $stmt = $sql->Execute($sql->Prepare("SELECT * FROM hms_patient_xraytest WHERE XT_CODE=".$sql->Param('a')),array($target));
	        print $sql->ErrorMsg();

	        if ($stmt->RecordCount()>0){
	            $xraytest_id = $stmt->FetchNextObject()->XT_ID;
	            $stmt = $sql->Execute($sql->Prepare("UPDATE hms_patient_xraytest SET XT_STATUS=".$sql->Param('a')." WHERE XT_CODE=".$sql->Param('b')),array('3',$target));
	            print $sql->ErrorMsg();
	            if ($stmt){
	                //  Event Log
	                $activity = 'X-ray Facility with code '.$faccode.' is been processed';
	                $engine->setEventLog('109',$activity);

	                // Clear Notifications
                    $engine->ClearNotification('070',$xraytest_id);
    ...
    */
    ...
    die('test');
        if(empty($target)){
            ...
            $stmt = $sql->execute($sql->Prepare("SELECT * FROM hms_patient_xraytest  WHERE XT_CODE = ".$sql->Param('1')." "),array($target));
            print $sql->Errormsg();
            ...
            if($stmt->Recordcount() > 0 ){
                ...
                $target = $obj->XT_CODE;
                $test= $encaes->decrypt($obj->XT_TESTNAME);
                $remarks= $encaes->decrypt($obj->XT_RMK);
            ...
            $stmtlist = $sql->execute($sql->Prepare("SELECT * FROM hms_patient_xraytest WHERE XT_VISITCODE = ".$sql->Param('1')." and XT_STATUS IN ('3','6')"),array($target));
            print $sql->Errormsg();
    ...
    case 'requestdetails':
        if(empty($keys)){
            ...
            $stmt = $sql->execute($sql->Prepare("SELECT * FROM hms_patient_xraytest JOIN hms_patient ON PATIENT_PATIENTCODE=XT_PATIENTCODE  WHERE XT_VISITCODE = ".$sql->Param('1')." and XT_STATUS IN ('3','6')"),array($keys));
            print $sql->Errormsg();
            ...
            if($stmt->Recordcount() > 0 ){
                ...
                $patientnum= $obj->XT_PATIENTNUM;
                $visitcode= $obj->XT_VISITCODE;
                $patientdate= $obj->XT_DATE;
                $test= $obj->XT_TESTNAME;
                $patientdob= $sql->UserDate($obj->PATIENT_DOB,'d-m-Y');
               
                
                $medic = $obj->XT_ACTORNAME;
            ...
            $stmtlist = $sql->execute($sql->Prepare("SELECT * FROM hms_patient_xraytest WHERE XT_VISITCODE = ".$sql->Param('1')." and XT_STATUS IN ('3','6')"),array($keys));
            print $sql->Errormsg();
...
if(!empty($fdsearch)){
    ...
    $query = "SELECT XTM_DATE,XTM_PATIENTNUM,XTM_PATIENTCODE,XTM_PATIENTNAME,XTM_VISITCODE,XTM_TOTAL_AMOUNT,XTM_ACTORNAME FROM hms_patient_xraytest_main WHERE  XTM_LABCODE = ".$sql->Param('a')." AND XTM_TYPE = '2' AND XTM_STATUS IN ('3','6') AND (XTM_PATIENTNAME LIKE ".$sql->Param('c')." OR XTM_PATIENTNUM LIKE ".$sql->Param('d')."  ) ";
    $input = array($faccode,'%'.$fdsearch.'%','%'.$fdsearch.'%');

}else{

	//echo $faccode;
    $query = "SELECT XTM_DATE,XTM_PATIENTNUM,XTM_PATIENTCODE,XTM_PATIENTNAME,XTM_VISITCODE,XTM_TOTAL_AMOUNT,XTM_ACTORNAME FROM hms_patient_xraytest_main WHERE XTM_STATUS IN ('3','6') AND XTM_TYPE = '2' AND XTM_LABCODE = ".$sql->Param('b')." ORDER BY XTM_DATE DESC";
    $input = array($faccode);
