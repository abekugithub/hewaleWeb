...
switch ($viewpage){
	...
	case 'testdetails':
        if(empty($keys)){
            ...
            $stmt = $sql->execute($sql->Prepare("SELECT * FROM hms_patient_xraytest JOIN hms_patient ON PATIENT_PATIENTCODE=XT_PATIENTCODE  WHERE XT_VISITCODE = ".$sql->Param('1')." and XT_STATUS IN ('1','6')"),array($keys));
            print $sql->Errormsg();
...
if(!empty($fdsearch)){
 $query = "SELECT DISTINCT XT_VISITCODE, XT_PATIENTNUM, XT_PATIENTNAME, XT_DATE,XT_ACTORNAME FROM hms_patient_xraytest WHERE  XT_LABCODE = ".$sql->Param('a')." and XT_STATUS IN ('1','6') AND (XT_PATIENTNAME LIKE ".$sql->Param('c')." OR XT_PATIENTNUM LIKE ".$sql->Param('d')."  ) ";
	...
	//echo $faccode;
    $query = "SELECT DISTINCT XT_VISITCODE, XT_PATIENTNUM, XT_PATIENTNAME, XT_DATE, XT_ACTORNAME FROM hms_patient_xraytest WHERE XT_STATUS IN ('1','6') AND XT_LABCODE = ".$sql->Param('b')." ORDER BY XT_DATE DESC";
    $input = array($faccode);
}
