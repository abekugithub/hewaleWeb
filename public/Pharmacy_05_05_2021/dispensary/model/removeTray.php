...
if (!empty($comcode)&&!empty($patientnum)&&!empty($patientcode)&&!empty($visitcode)){
    if ($type === 'Complains'){
        ...
        if ($stmt){
            ...
            if ($fetchstmt->RecordCount()>0){
                while ($obj = $fetchstmt->FetchNextObject()){
                    $result[]=array("<tr><td>".$num++."</td><td>".$encaes->decrypt($obj->PRESC_DRUG)."</td><td>".$obj->PRESC_DAYS."</td><td>".$obj->PRESC_FREQ."</td><td>".$obj->PRESC_TIMES."</td><td>".$obj->PRESC_ROUTENAME."</td><td><button type='button' id='deletecomplain' onclick='deleteComplains(\"".$obj->PRESC_CODE."\",\"Prescription\");' class=\"btn-danger removecomplain\">&times;</button></td></tr>");
                }
            }else{
                $result[] = array("");
            }
