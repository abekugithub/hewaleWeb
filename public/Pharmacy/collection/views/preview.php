<?php
  ...
  //  $report_comp_logo = $company->FACI_LOGO_ONAME;
    ...
    $stmt=$sql->Execute($sql->Prepare("SELECT SAL_DRUG,SAL_DOSAGE,SAL_QUANTITY,SAL_COST,SAL_NHIS,SAL_CUSTOMER,SAL_USERNAME,SAL_METHOD,SAL_CRDATE from hms_pharmacysales WHERE SAL_CODE=".$sql->Param('a')." AND SAL_FACICODE=".$sql->Param('b')." AND SAL_COST > ".$sql->Param('c').""),array($salcode,$faccode,'0'));
      if ($stmt->RecordCount()>0){
      	while ($obj=$stmt->FetchNextObject()){
      	$drugs = $encaes->decrypt($obj->SAL_DRUG);
		//$drugid = $encaes->decrypt($obj->PRESC_DRUGID);	
      	$date_rec=$obj->SAL_CRDATE;
      	$cart_rec[]=array('DRUG'=>$drugs,'DOSAGE'=>$obj->SAL_DOSAGE,'QUANTITY'=>$obj->SAL_QUANTITY,'NHIS'=>$obj->SAL_NHIS,'COST'=>$obj->SAL_COST,'CUSTOMER'=>$obj->SAL_CUSTOMER,'USERNAME'=>$obj->SAL_USERNAME,'METHOD'=>$obj->SAL_METHOD,'DATE'=>$obj->SAL_CRDATE);
      	}
      }
    ?>
            ...
            <table>
                <tr class="report-title">
                    ...
                    <td class="address" width="30%">
                        ...
                        <span><b>Date:</b> <?php echo $date_rec; ?></span><br><br>
                    </td>
                </tr>
                </table>
                <table>
